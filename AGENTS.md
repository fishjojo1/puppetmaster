...
- `skills/`: reusable prompt templates for Puppetmaster orchestrators and built-in `subagent-*.md` MCP skill entries. The project workflow uses `project-orchestrator.md` plus the planning/worktree review subagent family. The Google web VRP workflow uses `google-vrp-orchestrator.md` plus the `subagent-google-vrp-*` skill family. The ABBA IoT bug bounty workflow uses `abba-iot-bugbounty-orchestrator.md` plus the `subagent-abba-*` skill family.
