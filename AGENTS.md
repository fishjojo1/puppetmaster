...
- `skills/`: reusable prompt templates for Puppetmaster orchestrators and built-in `subagent-*.md` MCP skill entries. The generic project lifecycle workflow uses `project-orchestrator.md` plus the planning, worktree execution/review, code cleanup, final validation, and final audit subagent family. The Google web VRP workflow uses `google-vrp-orchestrator.md` plus the `subagent-google-vrp-*` skill family. The ABBA IoT bug bounty workflow uses `abba-iot-bugbounty-orchestrator.md` plus the `subagent-abba-*` skill family.
...
- `docs/google-vrp/web-methodologies.md`: current methodology research for safe, high-signal Google web VRP hypothesis generation and evidence standards.
...
Built-in subagent skill templates are discovered from `skills/subagent-*.md` and must include YAML frontmatter `description`. Root workflow prompts in `skills/`, such as `project-orchestrator.md`, `google-vrp-orchestrator.md`, `vuln-research-orchestrator.md`, and `abba-iot-bugbounty-orchestrator.md`, are not returned by `list_subagent_skills` because they are orchestrator prompts rather than child-agent roles. The project orchestrator workflow is intentionally tied to `planning/` artifacts, sibling git worktrees, serial milestones, two independent plan candidates, three execution candidates, independent candidate validation/review loops, candidate selection, post-merge cleanup, `subagent-final-validator`, and final audit. The Google web VRP workflow is tied to safe, documentation-first Google Bug Hunters policy and methodology artifacts and the `subagent-google-vrp-*` skill family.

Discord slash commands are guild-scoped. Channel bindings are the routing layer: one channel binds to one root orchestrator, and one root orchestrator binds to one channel. `/puppet bind` without `agent_id` presents a Discord select menu of live root orchestrators for mobile-friendly binding; the optional `agent_id` argument remains available for typed binding.

Discord `/skills` manages reusable prompts. With no arguments it lists saved skills, `skill-name` autocompletes from saved skills, with `skill-name` plus `prompt` it creates or updates a skill, with `skill-name` plus `view:true` it shows the stored prompt without running it, with only `skill-name` it sends the saved prompt to the channel's bound root orchestrator, with `skill-name` plus `extra-prompt` it appends one-off instructions when running the saved skill without modifying the saved prompt, and with `forget:true` it deletes the skill. Saved prompts that start with `/goal ` run as Codex goal-mode prompts with regenerated Puppetmaster runtime/tool instructions before the saved skill task.

The TUI opens on the agent tree and hides killed agents by default; press `K` to show or hide killed agents. Press `s` to switch to the reusable skills view, where `n` creates a skill, `e` or Enter edits the selected skill through `$VISUAL`/`$EDITOR`, and `d` deletes the selected skill.
