...
To give one root tree its own account pool, pass `--codex-home` multiple times or as a comma-delimited list:

```bash
puppet orchestrator start \
  --cwd /home/kek/Projects/project-a \
  --codex-home /path/to/codex-a,/path/to/codex-b \
  --prompt "Manage project A."
```

The root-scoped pool is stored with the root orchestrator in SQLite. Future child agents spawned by that root use the stored pool even if the global config or another root's pool differs.
...
When `home_pool` is set, Puppetmaster chooses the next source Codex home for each newly spawned agent and wraps back to the first entry after the end of the list. Each root records the active pool at startup, so MCP-spawned children keep using that root's pool. A single explicit `puppet orchestrator start --codex-home` or shell `CODEX_HOME=...` uses one source home for the root tree instead of a pool.
...
Each generated `codex-config/config.toml` starts from the selected source Codex home and overlays the per-agent hooks, project trust, and Puppetmaster MCP server settings. The source defaults to `~/.codex`, can be set globally with `[codex].home` in `~/.puppetmaster/config.toml`, can be rotated with `[codex].home_pool`, can be overridden for a root tree with one or more `puppet orchestrator start --codex-home` values, and honors `CODEX_HOME` when starting a root from the shell. Runtime `CODEX_HOME` is still per-agent so Puppetmaster can avoid mutating your global Codex config while giving each managed session its own hook trust and runtime state.
